...
| 🤖 **本地 AI 与大模型迁移** | Ollama 本地模型（`OLLAMA_MODELS`）、Hugging Face 权重（`HF_HOME`）官方环境变量原生换盘，解救几十 GB C 盘 |
| 🔍 **智能用户画像** | 家庭用户 / 开发者 / 游戏玩家 / 笔记本四类，一次画像不重复询问，后续推荐自动适配 |
| 🧹 **流氓与顽固服务根治** | 360 / 2345 / 百度网盘守护服务 / 迅雷 P2P / 搜狗弹窗 / WPS 四层复活链彻底封杀与卸载 |
| ⚡ **深度优化与 Win11 24H2** | 24H2 更新清理误报避坑、BitLocker 前置预检、服务精简、内存调优、电源计划、SSD TRIM |
| 🔄 **数据迁移** | 官方方案优先（应用内迁移/系统重定向/官方配置项），mklink 仅兜底；微信 4.x / QQ NT / 钉钉 / 浏览器专项 |
| 🧩 **自启动彻底化** | 覆盖 Windows 20+ 启动点，Edge 策略级锁定「先切开关层再删 Run 键」根治复发，推荐官方工具 AutoRuns |
| 📋 **15 个参考手册** | 涵盖磁盘清理、软件卸载、服务优化、自启动机制、官方迁移、微信/QQ/钉钉迁移、避坑库等全矩阵 |

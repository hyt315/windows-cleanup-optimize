...
| **🤖 本地 AI 与开发者环境换盘** | Ollama（`OLLAMA_MODELS`）、Hugging Face（`HF_HOME`）、ModelScope、Python uv、Android AVD 与 WSL2 稀疏磁盘原生缩容 | 原生释放几十到上百 GB C 盘空间 |
| **🔍 两种扫描入口** | 🅰️ 全量扫描（一键跑完 20 个模板，含 AI/VHDX 与 WMI/快捷方式劫持探测） + 🅱️ 画像扫描（家庭/游戏/开发/AI/笔记本五类） | 用户自选，非技术用户不被画像阻塞；专业用户拿到定制化建议 |
| **🧹 流氓软件与弹窗根治** | 刚需软件 IFEO 映像劫持底层静音、3秒活动弹窗秒级定位器、WMI 持久化常驻与快捷方式劫持审计、360/2345/WPS 四层复活链彻底封杀 | 治标更治本，让广告弹窗彻底消音 |
| **⚡ 深度优化与 Win11 适配** | 物理网卡 GUID 级 TCP Nagle 调优、Win11 Widgets 小组件/WebView2 后台抑制、传递优化清空、50+ 服务精简、SSD TRIM | 毫秒级提升系统网络响应与流畅度 |
| **🔄 官方原生数据迁移** | 应用内迁移 / 系统重定向优先，微信 4.x(及 3.x 孤岛清理) / 企业微信 / QQ NT / 钉钉专项迁移，`mklink` 仅做兜底 | 稳定合规，绝不破坏软件升级机制 |
| **📋 15 本全景参考手册** | 涵盖磁盘清理、软件卸载、服务精简、自启动机制、微信/QQ 迁移、86 条避坑库等全矩阵 | 深度知识沉淀，AI 调阅判断零失误 |
